...
object Bootstrap {
  ...
  object Modal {
    ...
    class Backend(t: BackendScope[Props, Unit]) {
      ...
      t.getDOMNode.map(dom => jQuery(dom.asMounted().asElement()).modal("hide")).void
    ...
    val component = ScalaComponent.builder[Props]("Modal")
      ...
      .componentDidMount(scope => Callback {
        ...
        jQuery(scope.getDOMNode.asMounted().asElement()).modal(js.Dynamic.literal("backdrop" -> p.backdrop, "keyboard" -> p.keyboard, "show" -> true))
        // register event listener to be notified when the modal is closed
        jQuery(scope.getDOMNode.asMounted().asElement()).on("hidden.bs.modal", null, null, scope.backend.hidden _)
      })
      .build
